...
def adaboost(test_data, train_data, iteration, saveTime, modelList):
    ...
    if modelList is not None:
        ...
        if saveTime:
            ...
            # modelList = [model(Rate=np.zeros((n_u, n_v), dtype=np.float32))]
        else:
    ...
    for t in range(iteration):
        ...
        if len(modelList)==0 and saveTime:
            ...
            precisionTR = get_accuracy([single_model], train_data, None,modelList[0].Rate)#write R
        ...
        precisionTR = get_accuracy([single_model], train_data, None)#write R
        single_model.alpha = 0.5 * np.log((u.data * (1 + precisionTR)).sum() / (u.data * (1 - precisionTR)).sum())
        if saveTime:
            # if t<1:
                # modelList[0].Rate *=single_model.alpha #R*alp
                # Parallel(n_jobs=NUM_THREADS, verbose=0, backend="threading")(delayed(single_model.get_rate)(i * BATCH_SIZE, min((i + 1) * BATCH_SIZE, n_u), modelList.Rate) for i in range(n_u // BATCH_SIZE + 1))
            # else:
...
def getRecList(ensemble, num_rec, trainR=None, test_users=None, availableItems=None):
    ...
    tops = Parallel(n_jobs=NUM_THREADS, verbose=0, backend="threading")(delayed(lambda x: get_top(*x)[:,:num_rec])([ensemble, i * BATCH_SIZE,
        min((i + 1) * BATCH_SIZE, m_num_users), trainR, None,test_users,  availableItems]) for i in range(n))
